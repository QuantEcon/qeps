...
than with anything we build. The problem it answers is **cost**, not safety.
...
2. **Disclose.** If AI tools meaningfully contributed to the code, tests, or
   ...
   Add it yourself; do not rely on the tool. A `Co-Authored-By:` trailer that a
   tool emits on its own (Claude Code and Copilot do) also serves as the marker,
   but an in-house or third-party pipeline emits nothing, so for those the
   trailer exists only if its operator writes it. The disclosure is a **marker,
   not a narrative**.
...
- **Decline AI-generated contributions on provenance grounds**, as
  ...
  outright as Gentoo has. Defensible positions that would cover our own workflow.
...
- **Detecting agents rather than asking.** Any mechanism keyed on recognising
  ...
  on a disclosure path the contributor can see, not on identifying agents.
...
**The disclosure path must be visible where contributions are made.** That means
a disclosure checkbox in the pull-request template, the `Assisted-by:` trailer
documented in `CONTRIBUTING.md` alongside the tool-emitted `Co-Authored-By:`
form it also accepts, and a link to this policy from the structured issue
templates that agents are already consuming — the same text several of them
echoed back to us verbatim. A norm nobody encounters is not a norm.
